...
double PID::CalcOutput(double actual) {
    return CalcOutput(actual, Util::GetMsecTime());
}
...
double PID::CalcOutput(double actual, uint32_t time) {
    ...
    if (m_timeLastUpdateSec != 0.0) {
        double deltaTimeSec = Util::GetSecTime() - m_timeLastUpdateSec;
    ...
    m_timeLastUpdateSec = Util::GetSecTime();
    m_prevPos = actual;

    output = m_Kp * error + std::clamp(m_Ki * m_integral, -m_icap, m_icap) + m_Kd * derivative;

    if (m_flags & Constants::PID_SPEED_CTRL) {
        output += m_lastOutput;
    }
...
double PID::CalcOutputWithError(double error) {
    return CalcOutputWithError(error, Util::GetMsecTime());
}
