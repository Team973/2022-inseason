...
void Greybot::EndCompetition() {
    int32_t status = 0;
    HAL_StopNotifier(m_notifier, &status);
}
Greybot::Greybot(double period) : Greybot(units::second_t(period)) {
}
Greybot::Greybot(units::second_t period) : IterativeRobotBase(period) {
    m_startTime = frc::Timer::GetFPGATimestamp();
    AddPeriodic([=] { LoopFunc(); }, period);
    int32_t status = 0;
    m_notifier = HAL_InitializeNotifier(&status);
    FRC_CheckErrorStatus(status, "{}", "InitializeNotifier");
    HAL_SetNotifierName(m_notifier, "Greybot", &status);
...
void Greybot::AddPeriodic(std::function<void()> callback, units::second_t period, units::second_t offset) {
